...
int joint0[180]; // array for servos
int joint1[180];
int joint2[180];
int joint3[180];
...
void setup()
{
  ...
  Serial.println("mini robot ready...");     
    Serial.println("By Sujay Alaspure");  
    Serial.println("Follow me on Insta For More Such Projects @alaspuresujay");  
  delay(1000);
  digitalWrite(13, LOW);
}
  ...
  {        
    if(currentMillis - previousMillis1 > 25) // 25miliseconds until next manual mode update
    {
      if (arrayStep < top) 
      {
        ...
        move_servo(); // set newservo position
        //record();   
      } // end counter < max
    } // end step check
  } // ende manualy move
